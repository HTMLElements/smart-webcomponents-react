...
export default function App() {
	...
	useEffect(() => {
		...
		for (let i = 0; i < currenciesData.length; i++) {
			...
			countryByCurrencyMap[data[2]] = codesData[i][1].toLowerCase();
		}
	...
	const columns = [
		{
			...
			template: (formatObject) => {
				...
				const flag = `
					<img 
						class="flag" 
						style="width:36px; height:24px;" src="https://www.htmlelements.com/demos/grid/live-update-countries/flags/${countryCode}.svg"
					/>`;

				if (!formatObject.template) {
					// render the template.
					formatObject.template =
						`<div className="template">${flag}<span>${value}</span></div>`;
				} else {
					// update the template.
					formatObject.template.children[0].src =
						`https://www.htmlelements.com/demos/grid/live-update-countries/flags/${countryCode}.svg`;

					formatObject.template.children[1].innerHTML = value;
				}
			}
		},
		{
			...
			template: function (formatObject) {
				...
				const value = new Number(formatObject.value.toString())
					.toFixed(2);
	...
	const updateValues = () => {
		...
		for (let i = 0; i < gridRef.current.rows.length; i++) {
			...
			const cell = gridRef.current.rows[i].cells[1];
			...
			if (index % 2 === 0) { value = cell.value + random(0.01, 0.3, 3); }
			else {
	...
	const random = (minValue, maxValue, precision) => {
