...
const Demo = () => {
  ...
  // To store imgSrc per row to avoid changing on every render
  const imgSrcMap = useRef(new Map());

  // Columns definition with JSX cellsRenderers
  const columns = [
    {
      label: "Employee",
      width: 400,
      ...
      cellsRenderer: (row, column, value, data) => {
        // Cache imgSrc per data.id to keep it stable
        if (!imgSrcMap.current.has(data.id)) {
          const randomNum = Math.floor(Math.random() * 70);
          imgSrcMap.current.set(
            data.id,
            `https://i.pravatar.cc/50?img=${randomNum}`
          );
        }
        const imgSrc = imgSrcMap.current.get(data.id);
        return `
          <div
    ...
    },
    {
      ...
      cellsRenderer: (row, column, value) => {
        ...
        const flag = `../../grid/live-update-countries/flags/${flags[value]}.svg`;
        return (
          <>
            <img
              loading="lazy"
      ...
      cellsRenderer: (row, column, value) => {
        return `
          <div className="contact-icons">
            <img
              src="https://img.icons8.com/ios-filled/50/00C851/linkedin.png"
              alt="LinkedIn"
			  width="20"
              style={{ cursor: "pointer" }}
            />
            <img
              src="https://img.icons8.com/ios-filled/50/ffbb33/new-post.png"
              alt="Email"
    		  width="20"
              style={{ cursor: "pointer" }}
            />
  ...
  useEffect(() => {
    if (treeGridRef.current) {
      treeGridRef.current.expandAllRows();
    }
  }, []);
