...
const rowsLocales = {
  ...
  fr: [
    [
      ...
      "Non commencé",
      "Élevée",
      "Capacité nécessaire pour 200 personnes",
    ],
  ...
  de: [
    [
      ...
      "Nicht gestartet",
      "Hoch",
      "Kapazität für 200 Personen benötigt",
    ],
  ...
  es: [
    [
      ...
      "No iniciado",
      "Alta",
      "Capacidad necesaria para 200 personas",
    ],
  ...
  it: [
    [
      ...
      "Non iniziato",
      "Alta",
      "Capacità necessaria per 200 persone",
    ],
  ...
  nl: [
    [
      ...
      "Niet gestart",
      "Hoog",
      "Capaciteit nodig voor 200 personen",
    ],
  ...
  pt: [
    [
      ...
      "Não iniciado",
      "Alta",
      "Capacidade necessária para 200 pessoas",
    ],
  ...
  ru: [
    [
      ...
      "Не начато",
      "Высокий",
      "Необходима вместимость на 200 человек",
    ],
...
const Demo = () => {
  ...
  useEffect(() => {
    ...
    // Initialize grid with English locale columns and data
    grid.columns = JSON.parse(JSON.stringify(columnsLocales["en"]));
    grid.dataSource = JSON.parse(JSON.stringify(rowsLocales["en"]));
    ...
    localizationRef.current.selectedIndexes = [0];
  }, []);

  const onLocalizationChange = async (event) => {
      const grid = gridRef.current;
      ...
      try {
        ...
        // Update columns and dataSource for the selected locale
        const columns = JSON.parse(JSON.stringify(columnsLocales))[locale];
        const dataSource = JSON.parse(JSON.stringify(rowsLocales))[locale];

        for (let i = 0; i < grid.columns.length; i++) {
          grid.columns[i].label = columns[i].label;

          if (columns[i].options) {
            grid.columns[i].options = columns[i].options;
          }
        }
  ...
  return (
    <div>
      <Grid id="grid" ref={gridRef} style={{ width: "calc(100% - 350px)", height: 600 }}></Grid>
