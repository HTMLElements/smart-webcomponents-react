...
// Data source as array of arrays (as in the original)
const rawData = [
...
const dataSource = rawData.map(row => {
  ...
  dataFields.forEach((field, i) => {
    obj[field.name] = field.dataType === "date" ? new Date(row[i]) : row[i];
  });
  return obj;
});

// Tag options for Seniority and Tags columns
...
// Last Interaction filter options
...
export default function Demo() {
  ...
  // Custom menu instance (for the menu button in the Name column)
                ...
                template: (formatObject) => {
                    ...
                    formatObject.template = [
                        ...
                        },
                        {
                            ...
                            onClick: (event, formatObject) => {
                                gridRef.current.beginEdit(formatObject.row.id);
                            }
                        },
                        {
                            className: 'menu',
                            ...
                            onClick: (event, formatObject) => {
                                ...
                                const menu = gridRef.current.nativeElement._customMenu || new Smart.Menu({
                                    dataSource: [
                                ...
                                gridRef.current.nativeElement._customMenu = menu;
            ...
            },
            {
                ...
                filterEditor: {
                    ...
                    onChange: function (event, input) {
                        ...
                        if (!value || value === '-') {
                            gridRef.current.removeFilter('LastInteraction');
                            return;
                        }
                        ...
                        gridRef.current.addFilter('LastInteraction', filterGroup);
                    }
                }
            },
            {
  ...
  return (
    <div id="container">
      <div className="demo-description">
        <h1>Leads Template</h1>
        <p>
          This demo illustrates how to create a leads template using the Data Grid component. A leads template is a
          predefined layout that allows users to quickly view and manage their leads in an organized manner. The
          template includes various columns such as lead name, contact information, status, and actions, providing
          users with a comprehensive overview of their leads.
        </p>
      </div>
      <Grid
        ref={gridRef}
