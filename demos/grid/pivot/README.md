# Smart UI for React - Data Grid Pivot
...
This example pins `smart-webcomponents-react@^27.0.0`. **The pivot view is not present in
earlier releases** - the component only gained `view: 'pivot'`, the `pivot` property bag,
`refreshPivot()`, `getPivotModel()` and `createPivotDesigner()` in 27.x.
...
| File | Purpose |
| --- | --- |
| `src/App.jsx` | The demo component |
| `src/bookings.js` | Deterministic data generator, pivot layout, heatmap scale, KPI maths |
| `src/App.css` | Pivot row, heatmap and layout styling |
| `src/page.css` | Page chrome, supplied by the demo site in the product docs |
