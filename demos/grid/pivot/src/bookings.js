...
// headline figures. Shared by App.jsx and App.tsx.
...
const createBookings = () => {
    ...
    const jitter = (spread) => 1 - spread + random() * spread * 2;
    const records = [];
...
const mix = (from, to, t) => '#' + from
    .map((channel, i) => Math.round(channel + (to[i] - channel) * t)
...
// the same pale shade.
export const computeHeatRanges = (model) => {
    const ranges = {};
...
// them out.
export const buildHeatmapRules = (model, ranges) => {
    ...
    for (const id of model.valueColumnIds) {
        ...
        const column = model.columns.find((candidate) => candidate.dataField === id),
...
export const computeKpis = (records) => {
    const total = (field, filter) => records
        .filter(filter || (() => true))
        .reduce((sum, booking) => sum + booking[field], 0);

    const revenue = total('revenue'),
        margin = total('margin'),
        deals = total('deals'),
        current = total('revenue', (booking) => booking.year === 2025),
        previous = total('revenue', (booking) => booking.year === 2024);
