...
class App extends React.Component {
	...
	},
	{
		...
		formatFunction: function (data, itemObj) {
			...
			if (data.length === 1) {
				...
				let result = '<div className="gantt-chart-task-assignees">';
	...
	async handleReady() {
		...
		for (let i = 0, max = resources.length; i < max; i += 1) {
			resourceTasks[resources[i].id] = await ganttChart.getResourceTasks(resources[i]);
		}

		ganttChart.resourceColumns = [{
			label: 'Name',
			value: 'label',
			size: '60%'
		},
		...
		return (
			<div>
				...
				<GanttChart ref={this.ganttChart} id="ganttChart" dataSource={this.dataSource} taskColumns={this.taskColumns}
					...
					resourceTimelineView={this.resourceTimelineView} resourceTimelineFormatFunction={this.resourceTimelineFormatFunction.bind(this)} onReady={this.handleReady.bind(this)}></GanttChart>
			</div>
		);
	}
}
