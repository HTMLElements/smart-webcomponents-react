...
class App extends React.Component {
	...
	async handleItemClick(event) {
		...
		if (target.classList.contains('add-task-button')) {
			...
			const itemPath = await ganttChart.getItemPath(eventDetails.item);
			const itemIndex = parseInt(itemPath.split('.').slice(-1)[0]) + 1;

			const taskProject = await ganttChart.getTaskProject(eventDetails.item);
			const itemProject = await ganttChart.getItemPath(taskProject);

			//Add a new Task
			const newItemId = await ganttChart.insertTask({
				label: 'New Task',
