...
export default function App() {
	...
	const countriesCodesData = GetCountriesCodesData();
	...
	const onColumnRender = (settings) => {
		if (settings.column.dataField === 'country') {
			...
			const countryCode = countriesCodesData
				.find(data => data[0]
					.indexOf(settings.text) === 0
				);
...
${settings.text}
	...
	function initPivotTable(data) {
		...
		pivottable.current.nativeElement.classList.remove('smart-visibility-hidden');
		pivottable.current.dataSource = new window.Smart.DataAdapter({
			dataSource: data,
			dataFields: [
				'ageGroup: string',
	...
	return (
		<div>
			...
			<h2 ref={loading} id="loading">Remote data is being loaded...</h2>
			<PivotTable
				ref={pivottable}
				id="pivotTable"
