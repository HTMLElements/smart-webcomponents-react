...
export default function App() {
	...
	const columns = [
		...
		},
		{
			...
			template: function (formatObject) {
				...
				if (formatObject.value < 50) { progressBarColor = 'bkg-red' }
				else if (formatObject.value < 80) { progressBarColor = 'bkg-yellow' }
				else { progressBarColor = 'bkg-green' }

				if (!formatObject.template) {

					const templateContainer = document.createElement('div');
					templateContainer.classList.add('cell-progress-bar-wrapper');

					const templateContainerRoot = createRoot(templateContainer);
					templateContainerRoot.render(
						<ProgressBar
							value={formatObject.value}
							className={`statement-progress-bar ${progressBarColor}`}
						/>
					)

					formatObject.template = templateContainer;

				} else if (formatObject.template.children[0]) {


					formatObject.template.children[0].value = formatObject.value;

					let progressBarClasses = formatObject.template.children[0].classList;
		...
		},
		{
			...
			cellsAlign: 'right',
			formatFunction: (settings) => setCellBackgroundColor(settings, 0, 4)
		},
		{
			label: '',
		...
		},
		{
			...
			cellsAlign: 'right',
			formatFunction: (settings) => setCellBackgroundColor(settings, 5, 6)
		},
		{
			label: '',
			dataField: 'Branches',
		...
		},
		{
			...
			cellsAlign: 'right',
			formatFunction: (settings) => setCellBackgroundColor(settings, 7, 8)
		},
		{
			label: '',
			dataField: 'Functions',
		...
		},
		{
			...
			cellsAlign: 'right',
			formatFunction: (settings) => setCellBackgroundColor(settings, 9, 10)
		},
		{
			label: '',
			dataField: 'Lines',
	...
	const setCellBackgroundColor = (settings, columnStart, columnEnd) => {

		const getBackgroundColor = (v) => {

			let cellColor;

			if (v < 50) { cellColor = '#FCE1E5' }
			else if (v < 80) { cellColor = '#fff4c2' }
			else { cellColor = '#E6F5D0' }
		...
		const cellColor = getBackgroundColor(value);

		for (let i = 0; i < settings.row.cells.length; i++) {

			if (i >= columnStart && i <= columnEnd) {

				settings.row.cells[i].background = cellColor;
				settings.row.cells[i].refresh();
			}
		}
	}
	...
	return (
		<div>
			<div className="header">
				<div className="path">
					<span className="home">all files </span>
					<span className="active">source/</span>
				</div>
				<div className="coverage-info">
					<div className="statements">
						<span className="percantage">77.24% </span>
						<span className="text">Statements </span>
						<span className="overview">22470/29090</span>
					</div>
					<div className="branches">
						<span className="percantage">67.32% </span>
						<span className="text">Branches </span>
						<span className="overview">14637/21741</span>
					</div>
					<div className="functions">
						<span className="percantage">84.02% </span>
						<span className="text">Functions </span>
						<span className="overview">1992/2371</span>
					</div>
					<div className="functions">
						<span className="percantage">77.31% </span>
						<span className="text">Lines </span>
						<span className="overview">22372/28938</span>
					</div>
				</div>
