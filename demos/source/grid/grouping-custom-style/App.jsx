...
export default function App() {
	...
	const gridSettings = {
		...
		grouping: {
			...
			formatFunction(fo) {
				console.log(fo);
				if (fo.row.groupDataField == 'Notification') {

					if (fo.value) {
						fo.value = "<i class='icon fa-bell-slash'></i>"
					} else {
						fo.value = "<i class='icon fa-bell'></i>"
					}
				}
		...
		dataSource: new window.Smart.DataAdapter({
			dataSource: gridData,
			groupBy: ['Name'],
			dataFields: gridDataFields.map(field => `${field.name}:${field.type}`)
		}),
		columns: [{
			label: 'Name',
			dataField: 'Name',
	...
	return (
		<div>
			...
			<div className="demo-description">
				...
				is applied when the value is changed. To group b' or 'Status', move
