...
function App() {
  ...
  // Generate employees data same as in the original code
  const employees = (() => {
    let data = [];
  ...
  useEffect(() => {
    ...
    formRef.current = new window.Smart.Form('#employeeForm', {
      value: employees[0]
    });
    ...
    if (employeeList) {
      employeeList.dataSource = employees.map((value) => value['First Name'] + ' ' + value['Last Name']);
      employeeList.selectedIndexes = [0];
    }
  // We only want this to run once after mount, so we only care about employees array
  ...
  const handleChange = useCallback(() => {
    if (formRef.current && employeeListRef.current) {
      const employeeIndex = employeeListRef.current.selectedIndexes[0];
      formRef.current.value = employees[employeeIndex];
    }
  }, [employees]);
  ...
  return (
    <div>
      ...
      <div id="employeeSelect">
        ...
        <DropDownList onChange={handleChange} ref={employeeListRef} id="employeeList"></DropDownList>
      </div>
      <br />
