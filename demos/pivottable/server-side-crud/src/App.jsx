...
export default function App() {
	...
	const dataSource = new window.Smart.DataAdapter({
		virtualDataSource: (resultCallbackFunction, details) => {
			if (details.action === 'dataBind') {
				...
				pivottable.current.dataSource = result.data; // syncing PivotTable's data source
			} else {
				const result = window.demoServer.getData(details);
				// logs the data below the component.
				LogData(result.data);
	...
	const handleTableClick = (e) => {
		...
		const tableElement = table.current.nativeElement;
		const rowId = deleteIcon.getAttribute('row-id');
		const tableRowObject = tableElement.rowById[rowId];

		tableElement.rows.splice(tableRowObject.data.$.index, 1);
	}

	const handleButtonClick = () => {
		table.current.nativeElement.rows.push({});
	}
