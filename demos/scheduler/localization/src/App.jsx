...
export default function App() {
	...
	const checkedDemoHeaderCheckBoxes = useRef({
		steven: true,
		mark: true,
		janet: true
	});
	...
	const dataSource = ['bg', 'en', 'de', 'fr'];

	const dropDownButtonPosition = 'right';

	const loadEvents = (initialize) => {

		const events = (initialize ? data : scheduler.current.events);
		const locale = scheduler.current.locale;
		...
		scheduler.current.dataSource = events;
	}

	const handleChange = (e) => {
		const locale = e.detail.value;

		scheduler.current.locale = locale;

		loadEvents();

		switch (locale) {
			case 'en':
				checkbox.current.innerHTML = 'Show Working Hours';
				break;
			case 'bg':
				checkbox.current.innerHTML = 'Покажи Работните Часове';
				break;
			case 'de':
				checkbox.current.innerHTML = 'Arbeitszeiten Anzeigen';
				break;
			case 'fr':
				checkbox.current.innerHTML = 'Afficher Les Heures De Travail';
				break;
		}
	}
	...
	const handleWorkingHours = (e) => {
		if (e.detail.value) {
			scheduler.current.hourStart = 8;
			scheduler.current.hourEnd = 16;
		}
		else {
			scheduler.current.hourStart = 0;
			scheduler.current.hourEnd = 23;
		}
	}

	const handleDemoHeaderCheckBoxChange = (checkbox, e) => {

		checkedDemoHeaderCheckBoxes.current[checkbox] = e.detail.value;

		const eventIds = Object.keys(checkedDemoHeaderCheckBoxes.current)
			.filter(k => checkedDemoHeaderCheckBoxes.current[k])

		scheduler.current.dataSource = data
			.filter(d => eventIds.indexOf(d.class) > -1);
				...
				<div className="demo-content">
					<div className="person-a">
						<CheckBox
							id="steven"
							checked
							onChange={handleDemoHeaderCheckBoxChange.bind(null, 'steven')}
						></CheckBox>
					</div>
					<div className="person-b">
						<CheckBox
							id="mark"
							checked
							onChange={handleDemoHeaderCheckBoxChange.bind(null, 'mark')}
						></CheckBox>
					</div>
					<div className="person-c">
						<CheckBox
							id="janet"
							checked
							onChange={handleDemoHeaderCheckBoxChange.bind(null, 'janet')}
						></CheckBox>
					</div>
				</div>
			</div>
			...
			<div className="options">
				...
				<div className="option">
					...
					<CheckBox
						...
						checked
						id="checkbox"
						onChange={handleWorkingHours.bind(this)}
					>Show Working Hours</CheckBox>
				</div>
			</div>
		</div>
	)
