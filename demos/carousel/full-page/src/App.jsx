...
const App = () => {
  const generateDataSource = (items) => {
    ...
    for (let i = 0; i < items; i++) {
      const paragraphIndex = Math.floor(Math.random() * contentParagraphs.length);
      const subParagraphLength = 300 + Math.random() * 100;
      const labelIndex = Math.floor(Math.random() * headlines.length);

      const item = {
        image: `./../../../src/images/carousel-large-${i + 1}.jpg`,
        label: headlines[labelIndex],
        content: contentParagraphs[paragraphIndex].substring(0, subParagraphLength) + '.'
      };
  ...
  return (
    <div>
      <Carousel dataSource={dataSource} keyboard slideShow autoPlay loop />
    </div>
  );
};
